...
export async function POST(
  ...
  return withPermission(request, "fiscal.cte.create", async (_user, ctx) => {
    try {
      ...
      if (isNaN(cteId)) {
        return NextResponse.json({ error: "ID de CTe inválido" }, { status: 400 });
      }
      ...
      if (!corrections || corrections.length === 0) {
        return NextResponse.json({ error: "Informe as correções" }, { status: 400 });
      }
      ...
      if (!cte) {
        return NextResponse.json({ error: "CTe não encontrado" }, { status: 404 });
      }
