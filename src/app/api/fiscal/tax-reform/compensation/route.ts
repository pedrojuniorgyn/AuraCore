 ...
 * 
 * Calcular compensação de créditos tributários
 * 
 * Body:
 * - periodStart: Data início do período (ISO)
 * - periodEnd: Data fim do período (ISO)
 * 
 * Response 200:
 * {
 *   success: true,
 *   data: {
 *     period: { start, end },
 *     currentCredits: { pis, cofins, total },
 *     newCredits: { ibs, cbs, total },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = calculateCompensationSchema.safeParse(body);
    ...
    // 4. Executar Use Case
    const useCase = new CalculateCompensationUseCase();
