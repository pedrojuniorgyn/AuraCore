import { NextRequest, NextResponse } from 'next/server';
import { withDI } from '@/shared/infrastructure/di/with-di';
import { getTenantContext } from '@/lib/auth/context';
import { resolveBranchIdOrThrow } from '@/lib/auth/branch';
import { auditTaxTransitionSchema } from '@/lib/validators/tax-reform';
import { AuditTaxTransitionUseCase } from '@/modules/fiscal/application/use-cases';
import { Result } from '@/shared/domain';
import { logger } from '@/shared/infrastructure/logging';

/**
 * POST /api/fiscal/tax-reform/audit
 * 
 * Registrar auditoria de transição tributária
 * 
 * Body:
 * - fiscalDocumentId: UUID do documento fiscal
 * - currentTaxes: { icms?, pis?, cofins?, ipi? }
 * - newTaxes: { ibsUf, ibsMun, cbs, is? }
 * 
 * Response 201:
 * {
 *   success: true,
 *   data: {
 *     auditId: string,
 *     createdAt: string
 *   }
 * }
 */
export const POST = withDI(async (request: NextRequest) => {
  try {
    // 1. Autenticação e Tenant Context
    const ctx = await getTenantContext();
    if (!ctx) {
      return NextResponse.json(
        { success: false, error: 'Não autenticado', code: 'UNAUTHORIZED' },
        { status: 401 }
      );
    }

    // 2. Branch Resolution
    const branchId = resolveBranchIdOrThrow(request.headers, ctx);

    // 3. Parse e Validação do Body
    const body = await request.json();
    const validation = auditTaxTransitionSchema.safeParse(body);
    
    if (!validation.success) {
      return NextResponse.json(
        {
          success: false,
          error: 'Dados inválidos',
          code: 'VALIDATION_ERROR',
          errors: validation.error.issues.map(e => ({
            path: e.path.join('.'),
            message: e.message,
          })),
        },
        { status: 400 }
      );
    }

    // 4. Executar Use Case
    const useCase = new AuditTaxTransitionUseCase({
      generate: () => crypto.randomUUID()
    });

    const result = await useCase.execute(
      {
        organizationId: ctx.organizationId,
        branchId,
        fiscalDocumentId: validation.data.fiscalDocumentId,
        currentTaxes: validation.data.currentTaxes,
        newTaxes: validation.data.newTaxes,
        calculatedBy: ctx.userId,
      },
      {
        userId: ctx.userId,
        organizationId: ctx.organizationId,
        branchId,
        isAdmin: false,
      }
    );

    // 5. Tratar Resultado
    if (Result.isFail(result)) {
      return NextResponse.json(
        { success: false, error: result.error, code: 'BUSINESS_ERROR' },
        { status: 400 }
      );
    }

    // 6. Retornar Sucesso (201 Created)
    return NextResponse.json(
      {
        success: true,
        data: result.value,
      },
      { status: 201 }
    );

  } catch (error) {
    // Propagar erros de auth (getTenantContext throws Response)
    if (error instanceof Response) {
      return error;
    }
    // 7. Error Handling
    logger.error('Error in POST /api/fiscal/tax-reform/audit', error);
    
    if (error instanceof Response) {
      return error;
    }
    
    return NextResponse.json(
      { success: false, error: 'Erro interno', code: 'INTERNAL_ERROR' },
      { status: 500 }
    );
  }
});

