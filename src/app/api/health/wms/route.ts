  ...
  try {
    ...
    try {
      ...
      metrics.totalLocations = Number(
        (locationsResult.recordset?.[0] as any)?.count ?? 0
      );
      ...
      metrics.totalStockItems = Number(
        (stockResult.recordset?.[0] as any)?.count ?? 0
      );
      ...
      metrics.totalMovements = Number(
        (movementsResult.recordset?.[0] as any)?.count ?? 0
      );
      ...
      metrics.pendingInventoryCounts = Number(
        (inventoryResult.recordset?.[0] as any)?.count ?? 0
      );
    } catch (e) {
