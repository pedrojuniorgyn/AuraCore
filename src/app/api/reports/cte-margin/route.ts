...
export async function GET(req: Request) {
  try {
    ...
    const totalVariableCosts = parseFloat((costsResult as any)[0]?.total_variable_costs || "0");
    ...
    const costBreakdown = (costDetailResult.recordset || []).map((row: any) => ({
