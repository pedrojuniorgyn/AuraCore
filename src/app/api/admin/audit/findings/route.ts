...
async function listFindings(
  ...
  try {
    ...
    const { runId, sinceDays, severity, limit } = parsed.data;
    const sinceDaysSafe = Number.isFinite(sinceDays ?? 0) ? (sinceDays ?? 0) : 0;
    const limitSafe = Number.isFinite(limit ?? 0) ? (limit ?? 200) : 200;

    const audit = await getAuditFinPool();

    // Necessário para aplicar scoping por tenant/filial com segurança
    ...
    const r = await audit
      ...
      .input("severity", (severity ?? null) as any)
...
export async function GET(req: NextRequest) {
  // Token (automação) OU permissão via sessão
