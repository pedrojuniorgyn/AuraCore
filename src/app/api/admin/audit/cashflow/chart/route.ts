  ...
  try {
    ...
    const r = await audit
      ...
      .query(
        ...
        GROUP BY CAST(d.data as date)
        ORDER BY CAST(d.data as date) ASC;
      `
      );

    const items = (r.recordset ?? []).map((row: any) => ({
      date: row.dia ? String(row.dia).slice(0, 10) : null,
      entradas: row.entradas == null ? 0 : Number(row.entradas),
      saidas: row.saidas == null ? 0 : Number(row.saidas),
      liquido: row.liquido == null ? 0 : Number(row.liquido),
    }));
    ...
    return NextResponse.json(
      { error: "Falha ao montar gráfico (cashflow)", ...(isProd && !debugRequested ? {} : { debug: { message } }) },
      { status: 500 }
    );
  }
}
