/**
 * API: POST /api/financial/receivables/ssrm
 * Server-Side Row Model para AG Grid - Contas a Receber
 * 
 * @module app/api/financial/receivables/ssrm
...
export async function POST(request: NextRequest) {
  try {
    ...
    const conditions: SQL[] = [
      ...
      eq(accountsReceivable.branchId, ctx.branchId),
    ...
    const rows = await (baseQuery as unknown as { limit: (n: number) => Promise<ReceivableSSRMRow[]> }).limit(pageSize);
    ...
    // Formatar resposta
    const response: IServerSideGetRowsResponse<ReceivableSSRMRow> = {
      rowData: rows as ReceivableSSRMRow[],
      rowCount: total,
    };
...
function getColumn(field: string): SQL | null {
  const columnMap: Record<string, SQL> = {
    documentNumber: accountsReceivable.documentNumber as unknown as SQL,
    partnerName: businessPartners.name as unknown as SQL,
    status: accountsReceivable.status as unknown as SQL,
    origin: accountsReceivable.origin as unknown as SQL,
    categoryName: financialCategories.name as unknown as SQL,
    description: accountsReceivable.description as unknown as SQL,
    amount: accountsReceivable.amount as unknown as SQL,
    amountReceived: accountsReceivable.amountReceived as unknown as SQL,
    dueDate: accountsReceivable.dueDate as unknown as SQL,
    issueDate: accountsReceivable.issueDate as unknown as SQL,
    createdAt: accountsReceivable.createdAt as unknown as SQL,
  };
...
function buildOrderBy(sortModel: Array<{ colId: string; sort: 'asc' | 'desc' }>): SQL[] {
  ...
  if (orderByClause.length === 0) {
    orderByClause.push(desc(accountsReceivable.createdAt as unknown as SQL));
  }
