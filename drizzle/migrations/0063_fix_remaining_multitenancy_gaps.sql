...
--
-- Tabelas afetadas:
-- 1. warehouse_inventory_counts - Adicionar branch_id, deleted_at
...
--
-- Rollback:
-- ALTER TABLE [warehouse_inventory_counts] DROP COLUMN [branch_id];
-- ALTER TABLE [warehouse_inventory_counts] DROP COLUMN [deleted_at];
...
-- ============================================
-- 1. warehouse_inventory_counts
-- ============================================

-- branch_id
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'warehouse_inventory_counts' AND COLUMN_NAME = 'branch_id')
BEGIN
    ALTER TABLE [warehouse_inventory_counts] ADD [branch_id] INT NOT NULL DEFAULT 1;
    PRINT 'Adicionado branch_id em warehouse_inventory_counts';
END
ELSE
BEGIN
    PRINT 'warehouse_inventory_counts ja tem branch_id';
END
GO

-- deleted_at
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'warehouse_inventory_counts' AND COLUMN_NAME = 'deleted_at')
BEGIN
    ALTER TABLE [warehouse_inventory_counts] ADD [deleted_at] DATETIME2 NULL;
    PRINT 'Adicionado deleted_at em warehouse_inventory_counts';
END
GO
...
BEGIN
    ...
    PRINT 'Criado indice idx_warehouse_inventory_counts_tenant';
END
GO
...
FROM [warehouse_inventory_counts] t
CROSS APPLY (
...
AND t.name IN ('warehouse_inventory_counts', 'inventory_adjustments')
