...
CREATE TABLE strategic_alert (
    ...
    -- Alert Type
    alert_type VARCHAR(50) NOT NULL, -- KPI_CRITICAL, VARIANCE_UNFAVORABLE, ACTION_PLAN_OVERDUE, GOAL_STALE
    severity VARCHAR(20) NOT NULL,   -- LOW, MEDIUM, HIGH, CRITICAL

    -- Related Entity
    entity_type VARCHAR(50) NOT NULL, -- KPI, GOAL, ACTION_PLAN
    ...
    -- Status
    status VARCHAR(20) NOT NULL DEFAULT 'PENDING', -- PENDING, SENT, ACKNOWLEDGED, DISMISSED
...
-- Indexes
CREATE INDEX IX_strategic_alert_org_branch
    ON strategic_alert(organization_id, branch_id);
CREATE INDEX IX_strategic_alert_status
    ON strategic_alert(status) WHERE status = 'PENDING';
CREATE INDEX IX_strategic_alert_entity
    ON strategic_alert(entity_type, entity_id);
CREATE INDEX IX_strategic_alert_created
    ON strategic_alert(created_at DESC);
GO
...
CREATE TABLE strategic_alert_config (
    ...
    -- KPI Thresholds
    kpi_critical_threshold DECIMAL(5,2) NOT NULL DEFAULT 70.00,  -- Below this = CRITICAL
    kpi_warning_threshold DECIMAL(5,2) NOT NULL DEFAULT 85.00,   -- Below this = WARNING

    -- Variance Thresholds
    variance_unfavorable_threshold DECIMAL(5,2) NOT NULL DEFAULT 15.00, -- Above this = alert
