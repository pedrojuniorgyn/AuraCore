# E8.1 - Audit de Índices SQL Server
...
| **Índices únicos existentes** | 19 |
| **Índices tenant (org+branch)** | **0** ⚠️ |

---

## 🔴 PROBLEMA CRÍTICO

**NENHUMA tabela possui índice composto (organizationId, branchId).**

Isso impacta TODAS as queries que filtram por tenant, causando:
- Full table scans em tabelas grandes
- Degradação de performance em produção
- Aumento de uso de memória do SQL Server

---

## 📋 TABELAS QUE PRECISAM DE ÍNDICE TENANT

### Prioridade CRÍTICA (organizationId + branchId presentes)

| Tabela | Campos | Índice Necessário |
|--------|--------|-------------------|
...
-- === ÍNDICES ADICIONAIS (filtros frequentes) ===
...
1. **Índices Filtered (WHERE deleted_at IS NULL)**: 
   - SQL Server suporta índices filtrados
   - Reduz tamanho do índice excluindo registros soft-deleted

2. **Ordem das Colunas**: 
   - `(organizationId, branchId)` é a ordem correta
   - Primeiro o campo mais seletivo (org), depois branch

3. **Impacto de Criação**:
   - Pode bloquear tabelas durante criação
   - Executar em horário de baixo uso ou usar ONLINE = ON

4. **Monitoramento Pós-Criação**:
...
| Operação | Antes | Depois (estimado) |
|----------|-------|-------------------|
| findMany com tenant filter | ~500ms | ~10ms |
| Count por organização | ~300ms | ~5ms |
